...
2. Connect Neon with prefix `NEON` so Vercel creates `NEON_DATABASE_URL`, then add the remaining values from `.env.example` in Vercel's Production environment. Do not commit `.env.local` or secrets.
